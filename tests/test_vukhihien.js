...
(async () => {
  ...
  for (const lop of LOP){
    ...
    r1[lop] = await p.evaluate((lop) => {
      ...
      const thanh = doc();
      const bo = nvBoGoc(player.sect, heroTier(player), gearVisual(player)) || '';
      return { ngoai, thanh, bo, tuMang: !!(window.NV_BO_CO_VK || {})[bo],
               daNhapNgoai: !!(ngoai && ngoai.nhap), oThanh: !!(thanh && !thanh.nhap) };
    }, lop);
  }
