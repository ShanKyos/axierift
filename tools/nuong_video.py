...
def rut(video, n, tmp):
    """Rút đúng n khung TRẢI ĐỀU cả đoạn — không phải n khung đầu."""
    ra = subprocess.run([FF, '-v', 'error', '-i', video, '-vf', f"thumbnail={max(1,n)}", '-frames:v', '1',
                         os.path.join(tmp, 'x.png')], capture_output=True)
    # thumbnail() không cho trải đều, nên đi đường chắc: đếm khung rồi select theo mod.
    p = subprocess.run([FF, '-v', 'error', '-i', video, '-map', '0:v:0', '-c', 'copy',
                        '-f', 'null', '-'], capture_output=True, text=True)
    subprocess.run([FF, '-v', 'error', '-i', video, '-vsync', '0',
                    os.path.join(tmp, 'f%04d.png')], check=True)
    fs = sorted(glob.glob(os.path.join(tmp, 'f*.png')))
    if not fs:
        raise SystemExit('không rút được khung nào')
    b = [fs[round(i * (len(fs) - 1) / max(1, n - 1))] for i in range(n)]
    return b, len(fs)
...
def chay(video, ra, khung=12, cot=4, cao=200, lam=360, chi_do=False):
    with tempfile.TemporaryDirectory() as tmp:
        fs, tong = rut(video, khung, tmp)
        print(f'{os.path.basename(video)} · {tong} khung → lấy {len(fs)}')
        anh, hop, nens, tams, liens = [], None, [], [], []
        for f in fs:
            im = Image.open(f)
            if im.height > lam:
                r = lam / im.height
                im = im.resize((round(im.width * r), lam), Image.LANCZOS)
            ...
            if bb:
                ...
                tams.append(((bb[0]+bb[2])/2, (bb[1]+bb[3])/2))
            anh.append(img)
        troi = 0 if len(tams) < 2 else max(max(abs(t[0]-tams[0][0]), abs(t[1]-tams[0][1])) for t in tams)
        print(f'  nền± {int(np.median(nens))} · trôi {troi:.0f}px · vụn {100*min(liens):.0f}% (khối lớn nhất, khung tệ nhất)')
        ...
        sheet.save(ra, lossless=False, quality=92, alpha_quality=100) if ra.endswith('.webp') else sheet.save(ra)
...
if __name__ == '__main__':
    ...
    chay(av[0], av[1] if len(av) > 1 and not av[1].startswith('--') else '/dev/null',
         g('--khung', 12), g('--cot', 4), g('--cao', 200), 360, '--do' in av)
