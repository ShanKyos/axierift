...
| # | Việc | Ở đâu |
|--:|---|---|
| ~~1~~ | ~~Cho `#combat-log .cl-row` xuống dòng~~ — **XONG** (xuống dòng + thụt đầu dòng treo) | `style.css` |
...
| ~~5~~ | ~~Bước hướng dẫn hết giờ thì DỪNG, đừng nhảy sang bước bất khả thi tại map hiện tại~~ — **XONG** (`duocO` + `tutLamDuoc`) | `game.js` `tutTick` |
